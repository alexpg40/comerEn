...
window.onload = () => {
    ...
    form.addEventListener('submit', evt => {
        ...
        if (validarCuenta()) {
            ...
            if (!validarNombre())
                handlerErrorEditar('El nombre debe tener 3 carácteres o más');
            if (!validarApellido())
                handlerErrorEditar('El apellido debe tener 3 carácteres o más');
            if (!validarCorreo())
                handlerErrorEditar('El formato del correo electrónico no es valido');
        }
    });
};
const validarCuenta = () => {
...
const handlerErrorEditar = (error) => {
...
//# sourceMappingURL=editarUsuario.js.map