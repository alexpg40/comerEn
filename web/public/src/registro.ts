...
window.onload = () => {
    ...
    form.addEventListener('submit', evt => {
        ...
        if(validarRegistro()){
            ...
            if(!validarNombre()) handlerErrorRegistro('El nombre debe tener 3 carácteres o más');
            if(!validarApellido()) handlerErrorRegistro('El apellido debe tener 3 carácteres o más');
            if(!validarCorreo()) handlerErrorRegistro('El formato del correo electrónico no es valido');
            if(!validarContraseña()) handlerErrorRegistro('La contraseña debe tener 8 caracteres y al menos un número');
            if(!validarRContraseña()) handlerErrorRegistro('La contraseña no coinciden');
        }
    })
}
...
const handlerErrorRegistro = (error : string) => {
