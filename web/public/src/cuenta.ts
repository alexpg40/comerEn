...
window.onload = () => {
    ...
    form.addEventListener('submit', evt => {
        ...
        if(validarCuenta()){
            ...
            if(!validarNombre()) handlerErrorCuenta('El nombre debe tener 3 carácteres o más');
            if(!validarApellido()) handlerErrorCuenta('El apellido debe tener 3 carácteres o más');
            if(!validarCorreo()) handlerErrorCuenta('El formato del correo electrónico no es valido');
            if(!validarContraseña()) handlerErrorCuenta('La contraseña debe tener 8 caracteres y al menos un número');
        }
    })
}
...
const handlerErrorCuenta= (error : string) => {
